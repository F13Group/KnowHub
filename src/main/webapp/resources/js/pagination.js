			...
			if (data.length == 0) {
				...
				$.each($(data).get().reverse(), function(index, value) {
					...
					var dd = unformatted_date.getDate();
		           	...
		           	if (userName) {
		           	$("<div class='divRow row'><div class='col-lg-6 col-md-6 col-sm-6 divQuestionColor divCell_2'>" + value.value + "</div><div class='col-lg-1 col-md-2 col-sm-2 divCell_Center'>" + value.category.shortValue + "</div><div class='col-lg-1 col-md-2 col-sm-2 divCell_Center'>" + date + "</div><div class='col-lg-2 col-md-2 col-sm-2 divCell_Center'>" + was_asked_button(value.id, value.isAsked) + " " + value.rating +  "</div><div class='col-lg-2 col-md-2 col-sm-2 divCell_Center'>" + "<img id=\"was_bookmarked\" src=\"resources/img/star.png\" width=\"20\" height=\"20\"/>" + "</div></div>").insertAfter("#headRow");
