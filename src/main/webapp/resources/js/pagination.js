...
if (globalCategoryUrl.indexOf("index") != -1) {
	globalCategoryUrl = globalCategoryUrl.substr(0, globalCategoryUrl
			.indexOf("index"));
}
	...
	$
			...
			.done(
					function(data) {
						...
						if (data.length == 0) {
							$(
									"<div class='divRow row'><div class='divCell_2 col-lg-12' align=center>No questions found for this category</div></div>")
									.insertAfter("#headRow");
							$("#pagingRow").hide();
						} else {
							$
									.each(
											...
											function(index, value) {
												...
												function was_asked_button(
														questionId, isAsked) {
													return '<img id="was_asked_button'
															+ questionId
															+ '" src="resources/img/rate.png" width="30" height="20" onclick="incrementQuestionRating('
															+ questionId
															+ ', '
															+ isAsked
															+ ')"  onmouseover="mouseOverWasAskedButton( '
															+ questionId
															+ ', '
															+ isAsked + ')">';
												}

												function was_bookmarked_button(
														questionId,
														isBookmarked) {
													var currentImg;
													var swapImg;
													...
													return '<img id="was_bookmarked'
															...
															+ '  data-swap='
															+ swapImg
															+ ' width="20" height="20" onmouseover="mouseOverWasBookmarkedButton('
															+ questionId
															+ ', '
															+ isBookmarked
															+ ')"/>';
												...
												if (userName) {
													$(
															"<div class='divRow row'><div class='non-active col-lg-6 col-md-6 col-sm-6 divCell_2'><a href='question/"
																	...
																	+ date
																	+ "</div><div class='col-lg-2 col-md-2 col-sm-2 divCell_Left'>"
																	+ was_asked_button(
																			value.id,
																			value.isAsked)
																	+ " "
													...
													$(
															"<div class='divRow row'><div class='col-lg-6 col-md-6 col-sm-6 divCell_2'><a href='question/"
																	+ value.id
																	+ "' class ='divQuestionColor'>"
																	+ value.value
		...
		.ready(
				function() {
					...
					if (questionUrl.indexOf("index") != -1) {
						questionUrl = questionUrl.substr(0, questionUrl
								.indexOf("index"));
					}
					if (questionUrl.indexOf("bookmarks") != -1) {
						questionUrl = questionUrl.substr(0, questionUrl
								.indexOf("bookmarks"));
					}
					...
					if ($("#categoriesMenu").length) {
						$("#categoriesMenu")
								.append(
										"<div class=categoriesMenuItem><input id=category-1 class=categoriesMenuButtonActive type=button value=All&nbsp;Categories onclick=selectCategory(-1) onmouseover=switchCategoryButtonOver('-1','true') onmouseout=switchCategoryButtonOver('-1','false')><br /></div>");
																...
																$(
																		...
																		.append(
																				"<div class=categoriesMenuItem><input id=category"
																						...
																						+ value.id
																						+ ") onmouseover=switchCategoryButtonOver('"
																						+ value.id
																						+ "','true') onmouseout=switchCategoryButtonOver('"
																						+ value.id
																						+ "','false')><br /></div>");
															});
										});
...
function toggleBookmark(questionId, isBookmarked) {
	...
	if (isBookmarked == false) {
		$.post(globalQuestionUrl + "/bookmark", {
			questionId : questionId
		}).done(function(isSuccess) {
			displayPage(currentPage);
		});
	} else if (isBookmarked == true) {
		$.post(globalQuestionUrl + "/unbookmark", {
			questionId : questionId
		}).done(function(isSucces) {
			displayPage(currentPage);
		});
	}
}
